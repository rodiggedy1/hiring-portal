...
export const hiringRouter = router({
    ...
    submitApplication: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        return { success: true, id: candidateId as number };
      }),
      ...
      .query(async ({ input }) => {
        ...
        return Promise.all(rows.map(async r => {
          ...
          return {
            ...
            videoUrl: r.videoUrl ? await storageGetSignedUrl(r.videoUrl.replace(/^\/manus-storage\//, "")).catch(() => r.videoUrl) : null,
            interviewVideoUrl: r.interviewVideoUrl ? await storageGetSignedUrl(r.interviewVideoUrl.replace(/^\/manus-storage\//, "")).catch(() => r.interviewVideoUrl) : null,
    ...
    sendCandidateMessage: agentProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const e164Phone = rawPhone.length === 10 ? `+1${rawPhone}` : `+${rawPhone}`;
        const smsResult = await sendSms({ to: e164Phone, content: input.message });
        return { success: true, sessionId: null, smsSent: smsResult.success };
      }),
    ...
    .query(async () => {
      ...
      const results = await Promise.all(
        realInterviewCandidates.map(async (cand) => {
          ...
          const sessions: { messageHistory: string }[] = [];
